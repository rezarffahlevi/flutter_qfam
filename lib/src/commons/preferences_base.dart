...
class Prefs {
  ...
  // AUTH
  static Future<AuthArgument?> get auth async {
    String pref = await PreferencesHelper.getString(KEY_AUTH);
    if (pref == '') return null;
    dynamic json = jsonDecode(pref);
    AuthArgument result = AuthArgument.fromJson(json);
    return result;
  }

  static Future setAuth(AuthArgument? value) async {
  ...
  static Future setToken(String? value) {
    if (value == null) return PreferencesHelper.removal(KEY_TOKEN);
    return PreferencesHelper.setString(KEY_TOKEN, value);
  }
